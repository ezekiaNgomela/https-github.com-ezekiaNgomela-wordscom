...
import { FileText, Image as ImageIcon, Search, Plus, MoreVertical, X, LayoutTemplate, Sparkles, User, Crown, Activity, Eye, Upload, FileSpreadsheet, Presentation, List } from 'lucide-react';
...
export function Dashboard({ onOpenFile }: { onOpenFile: (type: 'view-doc' | 'doc' | 'xls' | 'pdf' | 'ppt') => void }) {
  ...
  const [listDensity, setListDensity] = useState<'cozy' | 'compact'>('cozy');
  ...
  const renderFiles = () => (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-center px-4 py-3 bg-[#111111] gap-3 border-b border-white/5">
        ...
        <button
          onClick={() => setListDensity(prev => prev === 'cozy' ? 'compact' : 'cozy')}
          className="px-3 py-2 rounded-lg bg-[#1A1D20] text-gray-200 border border-white/10 text-xs font-medium hover:bg-[#25282c] transition-all"
          title="Toggle list density"
        >
          <span className="inline-flex items-center gap-2"><List size={14} /> {listDensity === 'cozy' ? 'Cozy' : 'Compact'}</span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 pb-24 lg:pb-6 text-left">
        ...
        {Object.entries(groupedFiles).map(([group, files]) => (
          <section key={group} className="mb-6 animate-in fade-in duration-300">
            ...
            <div className="bg-[#17191c]/90 backdrop-blur border border-white/10 rounded-2xl overflow-hidden shadow-[0_12px_30px_rgba(0,0,0,0.25)]">
              {files.map(f => (
                <div key={f.id} className={`group flex items-center justify-between ${listDensity === 'cozy' ? 'p-3.5' : 'p-2.5'} border-b last:border-b-0 border-white/5 hover:bg-white/5 transition-all duration-200`}>
                  ...
                  <div className="flex items-center gap-1">
                    {f.previewUrl && (
                      <button className="text-gray-300 hover:text-white p-2 rounded-lg hover:bg-white/10 transition-colors" onClick={() => setPreviewFile(f)} title="Preview">
                        <Eye size={16} />
                      </button>
                    )}
                    <button className="text-gray-500 p-2 rounded-lg hover:bg-white/10 hover:text-white transition-colors"><MoreVertical size={18} /></button>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="h-full bg-[#111111] text-gray-100 font-sans relative overflow-hidden">
      <div className="flex h-full">
        <aside className="hidden lg:flex w-64 xl:w-72 shrink-0 border-r border-white/10 bg-[#0d0e10] flex-col p-4 gap-3">
          ...
          {[
            ...
            <button key={item.id} onClick={() => setActiveNav(item.id as NavItem)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${activeNav === item.id ? 'bg-blue-500/20 text-blue-400 shadow-inner' : 'text-gray-300 hover:bg-white/5'}`}>
              {item.icon}<span>{item.label}</span>
            </button>
          ))}
          <button onClick={() => setIsCreateOpen(true)} className="mt-auto w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-blue-500 hover:bg-blue-600 active:scale-[0.99] transition-all text-white">
            <Plus size={18} /> Create
          </button>
        </aside>
